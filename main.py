...
async def youtube_to_txt(client, message: Message):
    ...
    # Fetch the YouTube information using yt-dlp with cookies --concurrent-fragments 5
    ydl_opts = {
        'quiet': True,
        'extract_flat': True,
        'skip_download': True,
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                cmd = f'yt-dlp -o  --concurrent-fragments 5"{name}.%(ext)s" -f "bestvideo[height<={raw_text2}]+bestaudio" --hls-prefer-ffmpeg --no-keep-video --remux-video mkv --no-warning  -threads 2"{url}"'
            ...
            if "jw-prod" in url:
                cmd = f'yt-dlp -o  --concurrent-fragments 5"{name}.mp4" "{url}"'
            elif "webvideos.classplusapp." in url:
               cmd = f'yt-dlp --add-header  --concurrent-fragments 5"referer:https://web.classplusapp.com/" --add-header "x-cdn-tag:empty" -f "{ytf}" "{url}" -o "{name}.mp4"'
            elif "youtube.com" in url or "youtu.be" in url:
                cmd = f'yt-dlp --cookies youtube_cookies.txt -f  --concurrent-fragments 5"{ytf}" "{url}" -o "{name}".mp4'
            else:
                cmd = f'yt-dlp -f  --concurrent-fragments 5"{ytf}" "{url}" -o "{name}.mp4"'
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    if "cwmediabkt99" in url:
                        ...
                        try:
                            cmd = f'yt-dlp -o  --concurrent-fragments 5"{name}.pdf" "{url}"'
                    ...
                    try:
                        ext = url.split('.')[-1]
                        cmd = f'yt-dlp -o  --concurrent-fragments 5"{name}.{ext}" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
    ...
    try:
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    try:
                        ext = url.split('.')[-1]
                        cmd = f'yt-dlp -x --audio-format {ext} -o  --concurrent-fragments 5"{name}.{ext}" "{url}"'
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
